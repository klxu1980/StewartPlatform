...
{
   // 机器人机械参数
   __JuneBugSpace.Robot.UpperArmLength   = CSpinEdit1->Value * 0.001;
   __JuneBugSpace.Robot.UpperBaseLength  = CSpinEdit2->Value * 0.001;
   __JuneBugSpace.Robot.LowerArmLength   = CSpinEdit3->Value * 0.001;
   __JuneBugSpace.Robot.LowerBaseLength  = CSpinEdit4->Value * 0.001;
   __JuneBugSpace.Robot.UpperJointHeight = 0;
   __JuneBugSpace.Robot.LowerJointHeight = 0;
   __JuneBugSpace.Robot.JackFixedLength  = CSpinEdit5->Value * 0.001;
   __JuneBugSpace.Robot.JackServoLength  = CSpinEdit6->Value * 0.001;
   __JuneBugSpace.Robot.JackServoBuffer  = 0;

   // 机器人姿态运动范围
   __JuneBugSpace.Z = CSpinEdit14->Value;
   __JuneBugSpace.YawRange = CSpinEdit13->Value;

   // 计算机器人运动范围
   __JuneBugSpace.SetSearchStep(1);

   // 搜索运动空间
   __JuneBugSpace.GetWorkSpaceBorder();
   if(!__JuneBugSpace.BorderXYZ.empty())
   {
      __WorkSpace = &__JuneBugSpace;
      __Space3DView.DrawSpace(&__JuneBugSpace, -__JuneBugSpace.BorderXYZ.front().Z);

      CSpinEdit17->MinValue = __JuneBugSpace.BorderXYZ.front().Z;
      CSpinEdit17->MaxValue = __JuneBugSpace.BorderXYZ.back().Z;
      CSpinEdit17->Value    = __JuneBugSpace.BorderXYZ.front().Z;
