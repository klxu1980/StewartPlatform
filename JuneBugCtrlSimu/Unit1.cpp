...
void TForm1::SendJuneBugCtrlOrder(void)
{
   ...
   float Speed    = CSpinEdit13->Value * 0.01;

   __RobotCommPkg.BuildJuneBugCtrlOrder(Dx, Dy, Yaw, Step, XYHeight, Speed);
   SendOrder(__RobotCommPkg);
}
//---------------------------------------------------------------------------
...
/*
   ...
   if(joyGetPosEx(JOYSTICKID1, &JoyInfo) == JOYERR_NOERROR)
   {
      ...
      float Alpha = 0.8;
      Dx = Alpha * __LstDx + (1 - Alpha) * Dx;
      Dy = Alpha * __LstDy + (1 - Alpha) * Dy;
      __LstDx = Dx;
      __LstDy = Dy;
      ...
      if(DD < 0.1)
         DD = 0;
      RefreshJoyStickIndicate(Dx, Dy, DD);

      __RobotCommPkg.BuildJuneBugCtrlOrder(Dx, Dy, 0, DD, 0.05);
      SendOrder(__RobotCommPkg);
   }
*/
}
...
//---------------------------------------------------------------------------
void TForm1::RefreshJoyStickIndicate(float Dx, float Dy, float Step)
{
   ...
   int X = Dx * Step * __BmpJoyStick->Width / 2;
   int Y = Dy * Step * __BmpJoyStick->Width / 2;
...
void __fastcall TForm1::Timer1Timer(TObject *Sender)
{
   if(CheckBox10->Checked)
   {
      ...
      float Step     = CSpinEdit11->Value * 0.01;
      float XYHeight = CSpinEdit12->Value * 0.001;
      float Speed    = CSpinEdit13->Value * 0.001;

      float Order[6] = {Dx, Dy, Yaw, Step, XYHeight, Speed};
      IdUDPClient1->Host = LabeledEdit30->Text;
      IdUDPClient1->Port = LabeledEdit31->Text.ToInt();
      IdUDPClient1->SendBuffer(Order, sizeof(float) * 6);
   }
}
//---------------------------------------------------------------------------
