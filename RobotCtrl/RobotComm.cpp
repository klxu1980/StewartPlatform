...
void CRobotCommPkg::BuildJuneBugCtrlOrder(float Dir, float Step, float Yaw, float XYHeight, float Speed)
{
   CtrlOrder[0] = 0x55;
   CtrlOrder[1] = 10;
   CtrlOrder[2] = JUNEBUG_CTRL;

   Short2Buffer(Dir      * 10.0,   CtrlOrder + 3);
   Short2Buffer(Step     * 1000.0, CtrlOrder + 5);
   Short2Buffer(Yaw      * 1000.0, CtrlOrder + 7);
   Short2Buffer(XYHeight * 1000.0, CtrlOrder + 9);
   Short2Buffer(Speed    * 1000.0, CtrlOrder + 11);
...
//---------------------------------------------------------------------------
void CRobotCommPkg::GetJuneBugOrder(float &Dir, float &Step, float &Yaw, float &XYHeight, float &Speed)
{
   Dir      = Buffer2Short(CtrlOrder + 3)  * 0.1;
   Step     = Buffer2Short(CtrlOrder + 5)  * 0.001;
