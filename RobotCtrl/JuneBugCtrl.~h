...
public:
   float MaxSpd[6];                    // 每个轴的最大速度和加速度上限，轨迹规划时对轨迹进行约束
   float MaxAcc[6];
...
private:
   ...
   void  CalcCubicParams(float T, float x0, float v0, float x1, float v1, float a[4]);
   ...
   void  ScheduleXYMovement(float XYHeight, float Dx, float Dy, float Yaw, float Step);
   void  ScheduleXYMovement(float XYHeight, float Dx, float Dy, float Yaw);
