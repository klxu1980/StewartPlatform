...
bool CRobot::CalcForceOfJacks(double Force[3], double Torque[3])
{
   ...
   // 通过求解线性方程获得每个连杆的力
   // 线性方程组的形式如下：
   // F[0]FV[0] + F[1]FV[1] + ... + F[5]FV[5] + Force  = 0    (连杆力的合力等于上平台所受力)
   // F[0]TV[0] + F[1]TV[1] + ... + F[5]TV[5] + Torque = 0    (连杆力矩的合力矩等于上平台所受力矩)
   Mat_DP M(6, 6);
   for(int i = 0; i < 6; ++i)
   {
      M[0][i] = FV[i][0];
   ...
   // 计算系数矩阵行列式，评估奇异性
   // 该计算主要是分析时使用
   ForceDet = CalcDeterminant(FV, TV);

   return rst;
}
//--------------------------------------------------------------------------
double CRobot::CalcDeterminant(double FV[6][3], double TV[6][3])
{
   Mat_DP M(6, 6);
   for(int i = 0; i < 6; ++i)
   {
