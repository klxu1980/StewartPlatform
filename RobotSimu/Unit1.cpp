...
void __fastcall TForm1::TimerCtrlSimuTimer(TObject *Sender)
{
   ...
   if(ListViewPostures->Items->Count)
   {
      ...
      for(int i = 0; i < 6; ++i)
      {
         ListViewJacks->Items->Item[i]->SubItems->Strings[0] = AnsiString().sprintf("%8.1f", __CurPosition[i] * SERVO_ROUND_MM / SERVO_PULSE_ROUND);
         ListViewJacks->Items->Item[i]->SubItems->Strings[1] = IntToStr(__PulseCnt[i]);
         ListViewJacks->Items->Item[i]->SubItems->Strings[2] = AnsiString().sprintf("%8.1f", __PulseCnt[i] * 60.0 / (SERVO_PULSE_ROUND * CTRL_INTERVAL) );
      }
      */
      #endif
      ...
      if(rst)
      {
         ...
         ListViewJacks->Items->Item[1]->SubItems->Strings[3] = AnsiString().sprintf("%8.3f", __CtrlSystem.Robot().Jack[1].Force);
         ListViewJacks->Items->Item[2]->SubItems->Strings[3] = AnsiString().sprintf("%8.3f", __CtrlSystem.Robot().Jack[2].Force);
         ListViewJacks->Items->Item[3]->SubItems->Strings[3] = AnsiString().sprintf("%8.3f", __CtrlSystem.Robot().Jack[3].Force);
         ListViewJacks->Items->Item[4]->SubItems->Strings[3] = AnsiString().sprintf("%8.3f", __CtrlSystem.Robot().Jack[4].Force);
         ListViewJacks->Items->Item[5]->SubItems->Strings[3] = AnsiString().sprintf("%8.3f", __CtrlSystem.Robot().Jack[5].Force);
      }
      else
      {
         ListViewJacks->Items->Item[0]->SubItems->Strings[3] = "S";
         ListViewJacks->Items->Item[1]->SubItems->Strings[3] = "S";
         ListViewJacks->Items->Item[2]->SubItems->Strings[3] = "S";
         ListViewJacks->Items->Item[3]->SubItems->Strings[3] = "S";
         ListViewJacks->Items->Item[4]->SubItems->Strings[3] = "S";
         ListViewJacks->Items->Item[5]->SubItems->Strings[3] = "S";
      }
      Edit2->Text = FloatToStr(__CtrlSystem.Robot().ForceDet);
   }
