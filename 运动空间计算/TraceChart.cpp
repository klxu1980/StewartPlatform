...
//---------------------------------------------------------------------------
CTraceChart::CTraceChart(CRobot &Robot)
 : __hdc(NULL),
   __XAngle(-90.0),
   __YAngle(0.0),
   __ZAngle(0.0),
...
// Initialize()
//---------------------------------------------------------------------------
void CTraceChart::Initialize(HDC hdc)
{
   __hdc = hdc;
...
//---------------------------------------------------------------------------
void CTraceChart::ResetProjection(void)
{
   // 设置投影方式
...
// Resize()
//---------------------------------------------------------------------------
void CTraceChart::Resize(int Left, int Top, int Width, int Height)
{
   __ScreenLeft   = Left;
   __ScreenTop    = Top;
   __ScreenWidth  = Width;
...
//---------------------------------------------------------------------------
void CTraceChart::SetProjectionMode(bool ProjMode)
{
   __ProjMode = ProjMode;
   Repaint();
}
...
// DrawBackground()
//---------------------------------------------------------------------------
void CTraceChart::DrawBackground(void)
{
   glLineWidth(3);
...
// RotateAroundX()
//---------------------------------------------------------------------------
void CTraceChart::RotateAroundX(double Angle)
{
   __XAngle += Angle;
   Repaint();
}
...
// RotateAroundY()
//---------------------------------------------------------------------------
void CTraceChart::RotateAroundY(double Angle)
{
   __YAngle += Angle;
   Repaint();
}
...
// RotateAroundZ()
//---------------------------------------------------------------------------
void CTraceChart::RotateAroundZ(double Angle)
{
   __ZAngle += Angle;
   Repaint();
}
//---------------------------------------------------------------------------
void CTraceChart::MoveScreen(double DeltaX, double DeltaY)
{
   __MiddleX += DeltaX;
   __MiddleY -= DeltaY;
...
//---------------------------------------------------------------------------
void CTraceChart::Zoom(double Factor)
{
   __AxisLength /= Factor;
   ResetProjection();
}
//---------------------------------------------------------------------------
void CTraceChart::DrawPlatform(double Platform[6][3], DWORD Color)
{
   glLineWidth(3);
...
//---------------------------------------------------------------------------
void CTraceChart::DrawLinks(CRobot &Robot, DWORD Color)
{
   glLineWidth(3);
   glColor3ub(Color & 0x0FF, (Color >> 8) & 0x0FF, (Color >> 16) & 0x0FF);
   ...
   glBegin(GL_LINES);
      for(int i = 0; i < 6; ++i)
      {
         glVertex3d(Robot.Links[i].UpperJoint[0], Robot.Links[i].UpperJoint[1], Robot.Links[i].UpperJoint[2]);
         glVertex3d(Robot.Links[i].LowerJoint[0], Robot.Links[i].LowerJoint[1], Robot.Links[i].LowerJoint[2]);
      }
   glEnd();
}
